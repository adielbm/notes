...
- (6.9) **Compactness Theorem** - Let $K$ be an infinite set of [[Math/Logic/Predicate Logic/Syntax#Sentence|sentences]]. If every finite subset of $K$ is [[Math/Logic/Predicate Logic/Semantic#Satisfiability|satisfiable]], then $K$ is satisfiable
- (6.10) **Löwenheim–Skolem theorem** - if a [[Math/Logic/Predicate Logic/Syntax#Theory|theory]] $K$ [[Math/Logic/Predicate Logic/Semantic#Truth Value in Model|has an infinite model]], then it has also countable model
...
Here are some example [[Math/Logic/Predicate Logic/Syntax#Theory|first-order theories]] that are important in mathematics. 
...
> See also: [[Math/Discrete Math/Set theory/Relations#Transitive Relations]] (strict partial order)
...
> See also: [[Math/Discrete Math/Set theory/Relations#Transitive Relations]] (strict total order)
...
- todo
	...
	- the relation $<$ is a [[Math/Discrete Math/Set theory/Relations#Transitive Relations|strict total order]] relation
	...
	- is $P$ a [[Math/Logic/Predicate Logic/Syntax#Theory|complete]] theory?
	- [[Math/Logic/Predicate Logic/Syntax#Theory|consistency]]
...
- (8.1) Let $M'=\langle B,\dots\rangle$ be a submodel of a model $M=\langle A,\dots\rangle$, and $a_{1},\dots,a_{n}\in B$, and $x_{1},\dots,x_{n}$ are variables. Let be [[Math/Logic/Predicate Logic/Syntax#Substitution|substitutions]] $S$ and $S'$ in $M$ and $M'$ respectively, s.t. $S(x_i)=S'(x_i)=a_i$ for all $i\leq n$. Then:
