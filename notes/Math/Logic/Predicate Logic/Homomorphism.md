...
- Let $\mathcal{M}$ and $\mathcal{N}$ [[Math/Logic/Predicate Logic/Semantic#Model|model]] in $L$, and $H\colon \mathcal{M}\to \mathcal{N}$ a function
	- We say that $H$ **preserves** the relations if for every $n$-ary [[Math/Logic/Predicate Logic/Syntax#Non-logical symbols (Signature)|predicate symbol]] $R$ in $L$, and for all $a_{1},\dots,a_{n}$ elements of the domain, we have:
	...
	- $H$ is a **homomorphism** of $\mathcal{M}$ into $\mathcal{N}$ if:
		-  ($H$ **preserves** constants) For every [[Math/Logic/Predicate Logic/Syntax#Terms|term]] $c$ in $L$, we have $H(c^{\mathcal{M}})=c^{\mathcal{N}}$ 
			- ( #todo in other book this goes on *constants* only)
		-  ($H$ **preserves** functions) For every $n$-ary [[Math/Logic/Predicate Logic/Syntax#Non-logical symbols (Signature)|function symbol]] $f$ and for all $a_{1},\dots,a_{n}$ elements of the domain $A$ we have: 
...
- A [[Math/Discrete Math/Set theory/Relations|binary relation]] $E$ on a [[Math/Logic/Predicate Logic/Semantic#Model|model]] $M$ in $L$ is called a **congruence (relation)** on $M$ if
	- $E$ is an [[Math/Discrete Math/Set theory/Relations#Equivalence relation|equivalence relation]]
