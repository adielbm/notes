...
- (4.4) **Soundness Theorem** (נאותות)
	- This proof calculus is [[Proof Calculus#Soundness|sound]], (i.e for all set of propositions $K$ and proposition $\psi$, If $K\vdash \varphi$ ([[#Proof Sequence|provable]]), then $K\models\varphi$ ([[Math/Logic/Propositional Logic/Semantic#Logical Implication|logically implied]]))
		- In particular, if $\emptyset\vdash \varphi$ then, $\varphi$ is a tautology ($\models \varphi$)
		- Corollary: If a set of propositions has a [[Math/Logic/Propositional Logic/Semantic#Model|model]], then it is consistent
...
- (4.7) **Completeness Theorem**
	...
	- For every theory $K$, there exists a [[Math/Logic/Propositional Logic/Semantic#Model|model]] $M$ such that $M \models K$ 
	- If $K \models \psi$ then $K \vdash \psi$ (i.e. if a theory $K$ [[Math/Logic/Propositional Logic/Semantic#Logical Implication|logically implies]] a proposition $\psi$, then $\psi$ is [[#Provable Proposition|provable]] from $K$)
...
| $K\vdash\varphi$<br>($\varphi$ is [[#Proof Sequence\|provable]] from $K$)                                               | $K\models\varphi$ (or $K \implies \psi$)<br>($K$ [[Math/Logic/Propositional Logic/Semantic#Logical Implication\|logically implies]] $\psi$) |
| $\vdash\varphi$ (or  $\emptyset\vdash \varphi$)<br>($\varphi$ is [[#Proof Sequence\|provable]] from the logical axioms) | $\models\varphi$<br>($\varphi$ is a [[Math/Logic/Propositional Logic/Semantic#Logical Validity\|tautology]])                                |
| $K$ is [[#Theory\|consistent]] (theory)                                                                                 | $M\models K$<br>($K$ has a [[Math/Logic/Propositional Logic/Semantic#Model\|model]] $M$ (or, $K$ is satisfiable))                           |
