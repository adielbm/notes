...
- **Second-order Logic** is an extension of [[Math/Logic/Predicate Logic/Syntax|first-order logic]].

- First-order logic [[Math/Logic/Predicate Logic/Syntax#Logical symbols|quantifies]] only variables that range over individuals, (elements of the [[Math/Logic/Predicate Logic/Semantic#Model|domain of discourse]]). 

- Let $\mathcal{L}$ be a [[Math/Logic/Predicate Logic/Syntax|first-order language]] over an [[Math/Logic/Predicate Logic/Syntax#Alphabet|alphabet]] $\mathcal{A}$, we define the **second-order (monadic) logic** $\mathcal{L}^2$ as the following:
