...
| ------------- | -------------------------------------------------------------------------- | ------- | --- |
| Union         | $L_1 \cup L_2 = \{w \mid w \in L_1 \lor w \in L_2\}$                       | Yes     |     |
| Intersection  | $L_1 \cap L_2 = \{w \mid w \in L_1 \land w \in L_2\}$                      | Yes     |     |
| Complement    | $\overline{L_1} = \{w \mid w \not\in L_1\}$                                | Yes     |     |
| Concatenation | $L_1 \cdot L_2 = \{wz \mid w \in L_1 \land z \in L_2\}$                    | Yes     |     |
| Kleene Star   | $L_1^{*} = \{\varepsilon\} \cup \{wz \mid w \in L_1 \land z \in L_1^{*}\}$ | Yes     |     |
| Reverse       | $L^\mathcal{R} = \{w^\mathcal{R} \mid w \in L\}$                           | Yes     |     |
...
- A **context-free grammar** (CFG) is a 4-tuple $G = (V, \Sigma, R, S)$ where
	- $V$ is a finite set of the **variables** (or **non-terminals**)
		- Symbolized by $A,B,\dots$
		- (In other areas, these are called _syntactic categories_)
	- $\Sigma$ is a finite set, disjoint from $V$, of the **terminals**
		- Symbolized by $a,b,\dots$
		- This is the alphabet of the language
	- $R$ is a finite set of **rules** (or **productions**) of the form $A\to w$, where $A\in V$ and $w\in (V\cup \Sigma)^*$
...
- #todo A CFG is ambiguous iff it generates some string with two different parse trees.
- Remarks:
