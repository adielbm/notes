- $\displaystyle I=\frac{dq}{dt}$ is the **electric current** (in $\mathsf{A}$) of a circuit in which a charge $q$ (in $\mathsf{C}$) passes through some surface in the circuit in an interval of time $t$ (in $\mathsf{s}$)
	- The SI unit of current is the **ampere** (abbreviated $\mathsf{amp}$ or $\mathsf{A}$), defined as $\mathsf{1\ A = 1\ C/s}$

- $\displaystyle\mathbf{\vec{J}}=\frac{I}{A}\hat{n}$ is the **current density** (in $\mathsf{A/m^2}$)
	- $A$ is the cross-sectional area (in $\mathsf{m^2}$)
